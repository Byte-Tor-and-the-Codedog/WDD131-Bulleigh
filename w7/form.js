...
form.addEventListener("submit", function (event) {
  ...
  if (isPastDate(availableDate)) {
    ...
    output.textContent = "Choose at least one campus";
  }
  else if (selectedCampuses.length > 1 && type === "one") {
    output.textContent = "Please select only one campus or change your preference to 'Two or more campuses'. ";
  }
  else if (type === "many" && selectedCampuses.length < 2) {
    output.textContent = "Choose more than one campus or change your preferences to 'one campus'.";
  }
