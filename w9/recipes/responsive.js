...
const searcher = document.querySelector("#search-btn");
...
function filterRecipes(recipe, recipeQuery) {
...
function render(recipe) {
    recipeBox.innerHTML += `
        ...
        <section class="info">
            ${recipe.tags.map(tag => `<button id="category">${tag}</button>`).join('')}
            <h1>${recipe.name}</h1>
            ${ratingTemplate(recipe.rating)}
            <p id="description">${recipe.desc}</p>
            </section>
        </section>
    `;
}
...
function runASearch() {
    ...
    let recipeResults = recipes.filter(recipe => filterRecipes(recipe, recipeQuery)).sort(compare);
    recipeResults.forEach(recipe => render(recipe)); 
};

searcher.addEventListener("click", runASearch);
